...
public class Network extends Thread {
    ...
    // Network buffer Semaphores
    private static Semaphore outputBufferSemaphore1;
    private static Semaphore outputBufferSemaphore2;
    private static Semaphore inputBufferSemaphore1;
    private static Semaphore inputBufferSemaphore2;
     ...
     */
     Network() {
         ...
         outputBufferSemaphore1 = new Semaphore(maxNbPackets);
         outputBufferSemaphore2 = new Semaphore(0);
         inputBufferSemaphore1 = new Semaphore(maxNbPackets);
         inputBufferSemaphore2 = new Semaphore(0);
    ...
    public static boolean send(Transactions inPacket) {
        ...
        try {
            ...
            if (getinputIndexClient() == getoutputIndexServer()) {
                ...
                System.out.println("\n DEBUG : Network.send() - inComingBuffer status normal");
            }

        } catch (InterruptedException e) {
            e.printStackTrace();
        } finally {
//            inputBufferSemaphore1.release();
            inputBufferSemaphore2.release();
            System.out.println("\ninputBufferSemaphore1.release(); " + inputBufferSemaphore1.availablePermits() + " permits available");
        }
     ...
     */
         public static boolean receive(Transactions outPacket) {
             ...
             try {
                 ...
                 outputBufferSemaphore2.acquire();
...
//                 outputBufferSemaphore2.release();
                 System.out.println("\nOutputBufferSemaphore2.release()");
             }
     ...
     */
         public static boolean transferOut(Transactions outPacket) {
            ...
            try {
                outputBufferSemaphore1.acquire();
                ...
                outputBufferSemaphore1.release();
                outputBufferSemaphore2.release();
                System.out.println("\n DEBUG: Network.transferOut() - outGoingBuffer status " + getOutBufferStatus());
            }
